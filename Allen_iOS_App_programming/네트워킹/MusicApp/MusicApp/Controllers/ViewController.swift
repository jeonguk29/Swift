...
final class ViewController: UIViewController {
    ...
    //<#T##UIViewController?#> 타입을 넘겨주면 됨 그러면 검색을 할때 여기에 등록한 뷰 컨트롤러를 자동으로 띄워주는 것임
    // - 그럼 검색할때 우리가 구현한 컬랙션 뷰가 보임
    
    let searchController = UISearchController(searchResultsController: UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "SearchResultViewController") as! SearchResultViewController)
    // 컨트롤러를 생성하는 코드 스토리 보드에서 Identifier 이름 같은지 확인 (설정해줘야함)
...
//        // 서치바 사용 테이블뷰 사용과똑같이 델리게이트 설정 필요함
        ...
        // - 검색 결과를 보여주기 위해 업데이트를 해주는애가 뷰컨이라고 설정
