...
final class ViewController: UIViewController {
    ...
    override func viewDidLayoutSubviews() { // LayoutSubviews를 다 마찬 시점
        super.viewDidLayoutSubviews()       // 버튼도 View 를 상속 받아만듬
        self.myButton.layer.cornerRadius = self.myButton.frame.width / 2
    }
    /*
     ⭐️ 동작 순서를 보면 View의 layoutSubviews이 호출이 된다음에 뷰컨 내부에 viewDidLayoutSubviews를 제공해줌
     */
    ...
    func setupConstraints() {
        ...
        myButton.centerXAnchor.constraint(equalTo: view.centerXAnchor).isActive = true      // x 축 정렬
        myButton.centerYAnchor.constraint(equalTo: view.centerYAnchor).isActive = true      // y 축 정렬
